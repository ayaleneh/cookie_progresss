...
cookie_store.prototype.jenerator=function(){
  ...
  for(let i=0;i<hours.length;i++){
    ...
    if(i<hours.length-1){
      ...
      this.arrayhour.push(assign)
      sum=sum+assign
    }
    else{
      row_data.innerText=sum
      this.arrayhour.push(sum)
    }
  }
...
let footer=function(){
  ...
  let f_head=document.createElement('th')
  f_row.appendChild(f_head)
  f_head.innerText="RESULT"
  for(let x=0;x<hours.length;x++){ 
    let counter=0
    for(let y=0;y<store.length;y++){
      counter=counter+store[y].arrayhour[x]
    }
    console.log(counter)
    table.appendChild(f_row)
    let f_data=document.createElement('td')
    f_row.appendChild(f_data)
    f_data.innerText=counter
  }
}
footer()
